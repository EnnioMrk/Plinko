...
class cvh_game {
  ...
  update_physics(dt) {
    this.pm?.update_physics(dt);
  }
  update_objects() {
    this.om.update_objects();
  }
  draw_objects() {
    // Draw QuadTree visualization if it exists

    if (this.pm?.draw_quad_tree && this.pm.quadTree) {
      this.pm.quadTree.draw(this.ctx);
    }
    this.om.draw_objects();
  }
  draw_loop(s, lastTime) {
    if (!this.running) return;
    const currentTime = performance.now();
    const dt = lastTime ? (currentTime - lastTime) / 1000 : 1 / 60;
    const fixedDt = 1 / 60; // Fixed timestep of 60 FPS

    if (s)
      setTimeout(
        () => requestAnimationFrame(() => this.draw_loop(s, currentTime)),
        s
      );
    else requestAnimationFrame(() => this.draw_loop(null, currentTime));

    this.clear_canvas();
    this.tick(fixedDt);
    this.update_physics(fixedDt);
    this.update_objects(fixedDt);
    this.draw_objects();
  }
  step(times = 1, dt = 1 / 60) {
...
class cvh_rectangle extends cvh_shape {
  ...
  draw(ctx) {
    ...
    if (this.text) {
      ...
      ctx.font = this.textFont || "14px Arial";
